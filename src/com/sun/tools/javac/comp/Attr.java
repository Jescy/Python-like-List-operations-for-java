 ...
 *************************************************************************/
    ...
    public void visitBlockExp(JCBlockExp betree) {
        ...
        if (env.info.scope.owner.kind == TYP) {
            ...
            localEnv.info.isBlockExp=true;

            attribStats(tree.stats, localEnv);
            localEnv.info.scope.leave();
        }
        result = check(betree, betree.getExpression().type, VAL, pkind, pt);
    }
    ...
    public void visitListComp(JCListComp tree) {
	    	...
	    	ClassType type= new ClassType(Type.noType, List.of(types.boxedTypeOrType(tree.expr.type)), syms.listType.tsym);
	    	tree.setType(type);
	    	result=type;
    ...
    public void visitNewList(JCNewList tree) {
        ...
        if(pt!=null &&pt!=Type.noType && pt.getTypeArguments().head!=null)
        {
        	elemType=pt.getTypeArguments().head;
        }
        if (tree.elems != null) {
        	for (JCExpression elem : tree.elems) {
				Type type=attribExpr(elem, env,elemType);			
			}
        	owntype=pt;
        }
        if(owntype==Type.noType)
        	owntype=new ClassType(Type.noType, List.of(syms.objectType), syms.listType.tsym);
       tree.setType(owntype);
       result=owntype;
    }
    ...
    public void visitBinary(JCBinary tree) {
        ...
        Type eleft=types.erasure(left);
        Type eright=types.erasure(right);
        ...
        if(types.isSubtypeUnchecked(eleft, syms.listType) 
        	...
        	|| types.isSameType(left, syms.intType) 
        		&& types.isSubtypeUnchecked(eright, syms.listType) 
        		&& tree.getOpcode()==JCTree.MUL
        	|| types.isSameType(right, syms.intType) 
    ...
    public void visitIndexed(JCArrayAccess tree) {
        ...
        {
        	Type rtype=types.erasure(atype.tsym.type);
        	if(types.isSubtypeUnchecked(rtype, syms.listType))
        	{
        		owntype=syms.objectType;
        		if(atype.getTypeArguments().head!=null)
        			owntype=atype.getTypeArguments().head;
        	}
